...
app.get("/filter",(req,res)=>{
  ...
  const filterJoke= jokes.filter((joke)=> joke.jokeType ===filter)
  res.json(filterJoke);
})
...
app.put("/jokes/:id",(req,res)=>{
  ...
  jokes[findJoke]= replacementJoke;
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  const searchIndex = jokes.findIndex((joke)=> joke.id===id);
   if(searchIndex > -1){
    ...
    res.status(404)
    .json({errror:`joke with id: ${id} not found. No jokes were deleted`});
...
app.get("/all",(req,res)=>{
  ...
  if(userKey===masterKey){
    ...
    res.sendStatus(404).json({errror:`Yo are not authorised to perform this action`})
  }
